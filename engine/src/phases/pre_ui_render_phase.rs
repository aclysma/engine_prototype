...
static PRE_UI_RENDER_PHASE_INDEX: AtomicI32 = AtomicI32::new(-1);

pub struct PreUiRenderPhase;

impl RenderPhase for PreUiRenderPhase {
    fn set_render_phase_index(index: RenderPhaseIndex) {
        PRE_UI_RENDER_PHASE_INDEX.store(index.try_into().unwrap(), Ordering::Release);
    }

    fn render_phase_index() -> RenderPhaseIndex {
        PRE_UI_RENDER_PHASE_INDEX.load(Ordering::Acquire) as RenderPhaseIndex
    }
